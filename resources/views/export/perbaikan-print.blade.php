...
<tr>
    ...
    <td class="value">
        @if (pathinfo($perbaikan->lampiran_perbaikan, PATHINFO_EXTENSION) === 'pdf')
            {{-- Merge with perbaikan-print if it's a PDF --}}
            @php
                $pdfMerger = new PDFMerger();
                $pdfMerger->addPDF(storage_path('app/public/' . $perbaikan->lampiran_perbaikan), 'all');
                $pdfMerger->addPDF(public_path('path/to/perbaikan-print.pdf'), 'all');
                $mergedPath = storage_path('app/public/merged/') . $perbaikan->tiket . '_merged.pdf';
                $pdfMerger->merge('file', $mergedPath);
            @endphp
            <embed src="{{ asset('storage/merged/' . $perbaikan->tiket . '_merged.pdf') }}" type="application/pdf" width="500px" height="300px">
        @else
            {{-- Display as an image if it's not a PDF --}}
            @php
                $imageData = file_get_contents(public_path('storage/' . $perbaikan->lampiran_perbaikan));
                $base64 = 'data:image/';
                $imageInfo = getimagesize(public_path('storage/' . $perbaikan->lampiran_perbaikan));
                $imageType = image_type_to_mime_type($imageInfo[2]);
                $base64 .= $imageType;
                $base64 .= ';base64,' . base64_encode($imageData);
            @endphp
